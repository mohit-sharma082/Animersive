...
class MegaCloud {
  ...
  decrypt(encrypted, keyOrSecret, maybe_iv) {
    ...
    if (maybe_iv) {
      key = CryptoJS.enc.Hex.parse(keyOrSecret);
      iv = CryptoJS.enc.Hex.parse(maybe_iv);
      contents = encrypted;
    } else {
      const cypher = CryptoJS.enc.Base64.parse(encrypted);
      const salt = CryptoJS.lib.WordArray.create(cypher.words.slice(2, 4));

      const password = CryptoJS.lib.WordArray.create(
        CryptoJS.enc.Latin1.parse(keyOrSecret).concat(salt)
      );
      ...
      for (let i = 0; i < 3; i++) {
        md5Hashes[i] = CryptoJS.MD5(digest);
        digest = md5Hashes[i].clone().concat(password);
      }

      key = md5Hashes[0].clone().concat(md5Hashes[1]);
      iv = md5Hashes[2];

      contents = CryptoJS.lib.WordArray.create(cypher.words.slice(4));
    }
